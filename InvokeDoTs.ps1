...
# Windows Update Log Analysis
    $wuLogPath = Join-Path $env:TEMP "WindowsUpdate_Analysis.log"
    Write-Log "Analyzing Windows Update logs..." "INFO"
    
    try {
        Get-WindowsUpdateLog -ETLPath "$env:windir\Logs\WindowsUpdate\etl" -LogPath $wuLogPath -ErrorAction Stop
        if (Test-Path $wuLogPath) {
            $wuLogContent = Get-Content $wuLogPath | Where-Object { 
                $_ -match "DeliveryOptimization|DO_|BITS|Windows Update" 
            }
            
            if ($wuLogContent) {
                Write-Log "Found $($wuLogContent.Count) DO-related entries in Windows Update logs" "INFO"
                
                # Add WU log entries to HealthCheck buffer with analysis
                foreach ($entry in $wuLogContent) {
                    $severity = "INFO"
                    if ($entry -match "error|fail|critical") {
                        $severity = "FAIL"
                        $issuesFound++
                    } elseif ($entry -match "warning|warn") {
                        $severity = "WARN"
                        $issuesFound++
                    }
                    ...
                    $Buffers.HealthCheck.Add([PSCustomObject]@{
                        Timestamp = Get-Date
                        Section = "Windows Update Logs"
                        Message = $entry
                        ResultType = $severity
                        Recommendation = if ($severity -eq "FAIL") {
                            "Review Windows Update error - may impact DO functionality"
                        } elseif ($severity -eq "WARN") {
                            "Consider investigating Windows Update warning"
                        } else { "" }
                    }) | Out-Null
                }
                
                # Add summary of Windows Update log analysis
                $Buffers.Summary.Add([PSCustomObject]@{
                    Test = "Windows Update Log Analysis"
                    Result = "$($wuLogContent.Count) DO-related entries found"
                    Status = if ($wuLogContent -match "error|fail|critical") { "WARN" } else { "PASS" }
                    Impact = "Additional diagnostic information from Windows Update logs"
                }) | Out-Null
            }
        }
    } catch {
        Write-Log "Failed to analyze Windows Update logs: $_" "WARN"
        $Buffers.Summary.Add([PSCustomObject]@{
            Test = "Windows Update Log Analysis"
            Result = "Analysis failed"
            Status = "WARN"
            Impact = "Could not analyze Windows Update logs"
        }) | Out-Null
    }
